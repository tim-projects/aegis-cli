...
def cli_main(stdscr, args, password):
    ...
    try:
        ...
        # Set nodelay to True and drain input buffer to prevent spurious Enter keypresses
        stdscr.nodelay(True)
        while True:
            ch = stdscr.getch()
            if ch == curses.ERR: # No more input
                break
        stdscr.nodelay(False) # Re-enable blocking getch for the main loop, or keep non-blocking and use select
                    ...
                    if args.uuid:
                        ...
                        if entry_to_reveal:
                            ...
                            # Call reveal mode directly. If it returns, and no group filter was specified, exit.
                            _, running, _ = run_reveal_mode(stdscr, initial_display_list[0], vault_data.db.otps, set(), get_ttn, config, max_rows, max_cols, curses_colors_enabled, initial_display_list, vault_data, NORMAL_TEXT_COLOR, HIGHLIGHT_COLOR, REVEAL_HIGHLIGHT_COLOR, RED_TEXT_COLOR, BOLD_WHITE_COLOR)
                            if not args.group: # If no group filter, then exit after showing single OTP
                                return
                            # If a group filter was specified alongside UUID, transition to search mode with filter
                            # This will be handled by passing args.group to run_search_mode
                    ...
                    # This loop handles search, group filtering, and selection
                    ...
                    if selected_otp_uuid:
                        ...
                        if entry_to_reveal:
                            ...
                            run_reveal_mode(stdscr, display_list_for_reveal[0], vault_data.db.otps, set(), get_ttn, config, max_rows, max_cols, curses_colors_enabled, display_list_for_reveal, vault_data, NORMAL_TEXT_COLOR, HIGHLIGHT_COLOR, REVEAL_HIGHLIGHT_COLOR, RED_TEXT_COLOR, BOLD_WHITE_COLOR)
                            # After reveal, the program exits, or returns to a higher level if we had a loop.
                            # For now, it will exit, as there's no loop to return to here.
                        else:
